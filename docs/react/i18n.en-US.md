...
The default language of `manyid@2.x` is currently English. If you wish to use other languages, follow the instructions below.

## ConfigProvider

`manyid` provides a React Component [ConfigProvider](/components/config-provider) for configuring manyid locale text globally.

```jsx
import { ConfigProvider } from 'manyid';
import frFR from 'manyid/locale/fr_FR';
...
1. Fork `manyid` and git clone to local, switch to `feature` branch, pull it to make sure it's up-to-date, create a new branch based on `feature` branch, all work will be done in it.
...
5. Update the `rc-picker` and `rc-pagination` versions in `manyid` and add the remaining other necessary content for the language. for example [Azerbaijani PR](https://github.com/ant-design/ant-design/pull/21387).
