...
使用 `create-react-app` 一步步地创建一个 TypeScript 项目，并引入 manyid。

> `manyid` 基于最新稳定版本的 TypeScript（`>=4.0.0`），请确保项目中使用匹配的版本。
...
修改 `src/App.tsx`，引入 manyid 的按钮组件。
...
重新启动 `yarn start`，现在你应该能看到页面上已经有了 manyid 的蓝色按钮组件，接下来就可以继续选用其他组件开发应用了。其他开发流程你可以参考 create-react-app 的[官方文档](https://create-react-app.dev/docs/getting-started#creating-a-typescript-app)。

`manyid` 使用 TypeScript 书写并提供了完整的定义，你可以享受组件属性输入建议和定义检查的功能。
