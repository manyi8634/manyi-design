...
Manyi Design React 致力于提供给程序员**愉悦**的开发体验。
...
这是一个最简单的 Manyi Design 组件的在线 codesandbox 演示。
...
直接用下面的代码替换 `index.js` 的内容，用 React 的方式直接使用 manyid 组件。

```jsx
import React, { useState } from 'react';
import { render } from 'react-dom';
import { ConfigProvider, DatePicker, message } from 'manyid';
// 由于 manyid 组件的默认文案是英文，所以需要修改为中文
...
```diff
- import { ConfigProvider, DatePicker, message } from 'manyid';
+ import { ConfigProvider, DatePicker, message, Alert } from 'manyid';
```
...
好的，现在你已经会使用基本的 manyid 组件了，你可以在这个例子中继续探索其他组件的用法。如果你遇到组件的 bug，也推荐建一个可重现的 codesandbox 来报告 bug。
...
`manyid` 默认支持基于 ES modules 的 tree shaking，直接引入 `import { Button } from 'manyid';` 就会有按需加载的效果。
...
目前社区也有很多基于 manyid 定制的 [React 脚手架](http://scaffold.ant.design/)，欢迎进行试用和贡献。
