...
Let's create a TypeScript project by using `create-react-app`, then import `manyid` step by step.

> We build `manyid` based on latest stable version of TypeScript (`>=4.0.0`), please make sure your project dependency matches it.
...
Create a new [cra-template-typescript](https://github.com/facebook/create-react-app/tree/master/packages/cra-template-typescript) project named `manyid-demo-ts` using yarn.

```bash
$ yarn create react-app manyid-demo-ts --template typescript
```

If you are using npm (we will use yarn in the following instructions, it's ok to replace yarn with npm)

```bash
$ npx create-react-app manyid-demo-ts --template typescript
```

Then we go inside `manyid-demo-ts` and start it.
...
Modify `src/App.tsx`, import Button component from `manyid`.
...
OK, reboot with `yarn start`, you should now see a blue primary button displayed on the page. Next you can choose any components of `manyid` to develop your application. Visit other workflows of `create-react-app` at it's [User Guide](https://create-react-app.dev/docs/getting-started#creating-a-typescript-app).

`manyid` is written in TypeScript with complete definitions, try out and enjoy the property suggestion and typing check.

![](https://gw.alipayobjects.com/zos/antfincdn/26L5vPoLug/8d7da796-175e-40af-8eea-e7031ba09f9f.png)

> Don't install `@types/manyid`.
...
- [Create React apps (with Typescript and manyid) with no build configuration](https://github.com/SZzzzz/react-scripts-ts-manyid)
